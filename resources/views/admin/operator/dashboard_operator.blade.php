...
@section('main')
    <section class="max-w-screen-xl mx-auto min-h-screen grid grid-cols-12 p-44 px-4 lg:px-12 gap-4">
        <div class="flex col-span-12 mb-2 mt-2" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-reverse rtl:space-x-reverse">
                <li class="inline-flex items-center">
                    <a class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-color-primary-600 dark:text-gray-400 dark:hover:text-white">
                ...
                <div>
                    <form id="filter-form" class="w-full grid grid-cols-7 gap-4">
                        <div class="relative mb-2 col-span-12 lg:col-span-3">
                            ...
                            <select id="input-group-1" class="bg-gray-50 border block border-gray-300 text-gray-900 xl:text-sm text-xs rounded-lg w-full ps-12 p-4">
                        ...
                        <div class="relative mb-2 col-span-12 lg:col-span-3">
                            ...
                            <select id="input-group-2" class="bg-gray-50 border block border-gray-300 text-gray-900 xl:text-sm text-xs rounded-lg w-full ps-10 p-4">
                                <option value="">Pilih Tahun Akademik</option>
                            </select>
                        </div>
                        <div class="col-span-3 lg:col-span-1">
                            <button type="button" id="filter-button" class="text-white w-full h-fit bg-color-primary-500 hover:bg-color-primary-600 focus:ring-4 focus:ring-color-primary-300 font-medium rounded-lg text-base px-5 py-3.5 me-2">
                                Cari
                            </button>
                        </div>
                    </form>
        ...
        <div class="lg:col-span-4 col-span-12 w-full ">
            <p class="text-sm italic my-4">Total Lowongan : <span id="total-lowongan">{{ $data['lowongan']->count() }}</span></p>
            <div id="lowongan-list" class="w-full max-h-[42rem] overflow-y-auto flex flex-col gap-y-4">
                @foreach ($data['lowongan'] as $item)
                    <div onclick="showProgramDetail({{$item->id}})" data-program-id="{{ $item->program->id }}" data-tahun-akademik="{{ $item->tahun_akademik }}" class="lowongan-item w-full bg-white border border-gray-200 hover:border-color-primary-500 hover:bg-slate-100 transition-colors duration-300 rounded-lg shadow">
                        <a class="w-full">
                            ...
                            <div class="">
                                <div class="flex justify-between items-center">
                                    <div class="flex gap-x-2 items-center text-color-primary-500">
                                        <span class=""><i class="fas fa-book text-sm"></i>{{ $item->program->name }}</span>
                                ...
                                <p class="text-sm mt-1 text-slate-500">{{ $item->tahun_akademik . ' (' . $item->semester . ')' }}</p>
                            </div>
                        </div>
                    </div>
                @endforeach
