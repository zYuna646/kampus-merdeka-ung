...
@section('main')
    <section class="max-w-screen-xl min-h-screen mx-auto grid grid-cols-12 py-44 px-4 lg:px-12 gap-4">
      ...
      <h4 class="font-semibold text-lg">Lorem ipsum dolor, sit amet</h4>
        ...
        @foreach ($data->programTransaction as $item)

                <div class="max-h-[42rem] overflow-y-auto flex flex-col p-2"
                    onclick="showPesertaDetail({{ $item->id }})>
                    <div
                        class="relative
                    hover:cursor-pointer overflow-visible bg-white p-6 rounded-xl w-full flex gap-x-4 border
                    border-slate-200 shadow-sm hover:border-color-primary-500 hover:bg-slate-50 transition-all
                    duration-300">
