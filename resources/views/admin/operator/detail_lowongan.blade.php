...
<section class="max-w-screen-xl min-h-screen mx-auto grid grid-cols-12 py-44 px-4 lg:px-12 gap-4">
    ...
    <div class="col-span-12 grid grid-cols-12 p-10 bg-white rounded-xl border border-slate-200 shadow-sm">
        ...
        <div class="col-span-12 mt-4 flex flex-col gap-y-2">
            ...
            <div class="flex flex-col">
                ...
                $data['lowongan']->tanggal_mulai,
                );
                $tanggalSelesai = \Carbon\Carbon::parse(
                $data['lowongan']->tanggal_selesai,
                );
    ...
    <div class="col-span-12 lg:col-span-4">
        <div id="pesertaList" class="max-h-[42rem] overflow-y-auto flex flex-col p-2">
            @foreach ($data['peserta'] as $peserta)
            <div class="peserta-item relative hover:cursor-pointer overflow-visible bg-white p-6 rounded-xl w-full flex gap-x-4 border border-slate-200 shadow-sm hover:border-color-primary-500 hover:bg-slate-50 transition-all duration-300"
                data-peserta-id="{{ $peserta->id }}" data-location-id="{{ $peserta->lokasi_id }}" data-name="{{ $peserta->mahasiswa->name }}" data-nim="{{ $peserta->mahasiswa->nim }}">
...
<script>
    ...
    function filterPeserta() {
        ...
        document.querySelectorAll('.peserta-item').forEach(function(item) {
            ...
            if ((name.includes(searchValue) || nim.includes(searchValue)) && (locationValue === "" || location === locationValue)) {
    ...
    document.getElementById('search').addEventListener('input', filterPeserta);
    document.getElementById('location').addEventListener('change', filterPeserta);

    document.querySelectorAll('.peserta-item').forEach(function(item) {
        item.addEventListener('click', function() {
            let pesertaId = this.getAttribute('data-peserta-id');
