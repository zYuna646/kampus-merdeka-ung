...
<section class="max-w-screen-xl mx-auto min-h-screen flex flex-col py-44 px-4 lg:px-12 gap-4">
    ...
    <div class="flex justify-between lg:flex-row flex-col lg:items-center gap-y-4">
        ...
        <div class="inline-flex flex-wrap gap-2">
            <x-button_md type="button" onclick="window.location.href='{{ route('admin.guru.add') }}'">
            ...
            <!-- Tombol Export -->
            <x-button_md type="button" color="warning"
                            ...
                            <div class="relative inline-block text-left">
                                ...
                                <div id="dropdownMenu{{ $item->id }}"
                                    ...
                                    <div class="py-1" role="none">
                                        <button data-id="{{ $item->id }}" data-nik="{{ $item->nik }}"
                                            data-name="{{ $item->name }}"
                                            data-lokasis="{{ json_encode($item->lokasis) }}" onclick="modalOpen(this)"
                                        ...
                                        <form action="{{ route('admin.guru.delete', $item->id) }}" method="POST" class="w-full"
                                ...
                                <div class=" w-full p-6 bg-white rounded-xl">
                                    ...
                                    <div class="w-full inline-flex mt-4 gap-x-1">
                                        <x-button_sm class="inline-flex items-center gap-x-2" color="info"
                                            onclick="window.location.href='{{ route('admin.guru.edit', $item->id) }}'">
                                            <span><i class="fas fa-edit"></i></span>
                                            Edit
                                        </x-button_sm>
                                        <form action="{{ route('admin.gurup.delete', $item->id) }}" method="POST"
                                            role="none" style="display: inline-block;">
                                            @csrf
                                            @method('DELETE')
...
<script>
    const importBtn = document.getElementById('importBtn');
        ...
        const lokasis = JSON.parse(button.getAttribute('data-lokasis'));
        ...
        let lokasisHtml = '';
        lokasis.forEach((lokasi, index) => {
            const provinsi = lokasi.provinsi ? lokasi.provinsi.name : 'N/A';
            const kabupaten = lokasi.kabupaten ? lokasi.kabupaten.name : 'N/A';
            const kecamatan = lokasi.kecamatan ? lokasi.kecamatan.name : 'N/A';
            const kelurahan = lokasi.kelurahan ? lokasi.kelurahan.name : 'N/A';
            const address = `${kelurahan}, ${kecamatan}, ${kabupaten}, ${provinsi}`;

            lokasisHtml += `
                <li class="">
                    ${index + 1}. ${lokasi.name} - ${address}
                </li>`;
        });
        ...
        modalContent.innerHTML = `
            ...
            <div class="flex flex-col gap-y-px">
                <p class="font-semibold">Lokasi Program</p>
                <ul class="flex flex-col gap-y-2 lower-case">${lokasisHtml}</ul>
            </div>
        `;
