...
@section('main')
    <section class="max-w-screen-lg mx-auto min-h-screen flex flex-col py-12 px-4 lg:px-12 gap-4">
        <div class="bg-white p-6 rounded-xl mt-32">
            <h2 class="text-xl font-semibold mb-4">Edit Pamong</h2>
            <form action="{{ route('admin.pamong.update', $data['pamong']->id) }}" method="POST">
                @csrf
                <div class="mb-4">
                    ...
                    <select name="dosen_id" id="lokasi"
                        ...
                        required disabled>
                        <option selected value="{{ $data['pamong']->id }}">
                            {{ $data['pamong']->guru->nik . ' - ' . $data['pamong']->guru->name }}</option>
                ...
                <div class="mb-4">
                    ...
                    <select name="lowongan_id" id="lokasi"
                        class="block w-full xl:p-4 p-3 text-gray-900 border border-gray-300 rounded-md bg-gray-50 xl:text-sm text-xs"
                        required disabled>
                        <option selected value="{{ $data['pamong']->lowongan->id }}">
                            {{ $data['pamong']->lowongan->program->name . ' (' . $data['pamong']->lowongan->tahun_akademik . ') - ' . $data['pamong']->lowongan->semester }}
                        </option>
                        <!-- Ganti dengan data lokasi yang sesuai -->
                    </select>
                </div>
                <div class="mb-4">
                    ...
                    <select multiple="multiple" class="js-example-basic-multiple w-full" name="lokasi_id[]" disabled>
                        @foreach ($data['pamong']->guru->lokasis as $item)
