        ...
        <form action="{{ route('admin.lowongan.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="tahun_akademik" id="tahun_akademik"
                    ...
                    $endYear = 2050;
                    @endphp
                    @for ($year = $startYear; $year <= $endYear; $year++) <option value="{{ $year }}-{{ $year + 1 }}">{{
                        $year }} - {{ $year + 1 }}</option>
                        @endfor
                </select>
            </div>
                ...
                <div class="mt-2">
                    <label class="inline-flex items-center">
                        <input type="radio" name="isLogBook" value="1"
                            class="form-radio text-indigo-600 focus:ring-indigo-500 h-4 w-4" required checked>
                        <span class="ml-2 text-sm text-gray-900">Ya</span>
                    </label>
                    <label class="inline-flex items-center ml-6">
                        <input type="radio" name="isLogBook" value="0"
                            class="form-radio text-red-600 focus:ring-red-500 h-4 w-4" required>
            ...
            <div class="mb-4">
                <label for="pendaftaran_mulai" class="block text-sm font-medium text-gray-700 mb-2">Pendaftaran
                    Mulai</label>
            ...
            <div class="mb-4">
                <label for="pendaftaran_selesai" class="block text-sm font-medium text-gray-700 mb-2">Pendaftaran
                    Selesai</label>
                <input type="datetime-local" name="pendaftaran_selesai" id="pendaftran_selesai"
                    class="block w-full xl:p-4 p-3 text-gray-900 border border-gray-300 rounded-md bg-gray-50 xl:text-sm text-xs"
                    required>
            </div>
            <div class="mb-4">
            ...
            <button type="submit"
                class="text-white w-full h-full bg-color-primary-500 hover:bg-color-primary-600 focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 col-span-12 mt-2">
                Kirim
            </button>
        </form>
    </div>
