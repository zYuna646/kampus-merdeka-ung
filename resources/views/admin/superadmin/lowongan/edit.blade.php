        ...
        <form action="{{ route('admin.lowongan.update', $data['lowongan']->id) }}" method="POST"
            ...
            <div class="mb-4">
                ...
                <select name="program_id" id="program"
                    ...
                    <option value="{{ $program->id }}" {{$data['lowongan']->program_id == $program->id ? 'selected' :
                        ''}}>{{ $program->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                ...
                <select name="tahun_akademik" id="tahun_akademik"
                    ...
                    @for ($year = $startYear; $year <= $endYear; $year++) <option value="{{ $year }}-{{ $year + 1 }}" {{
                        $data['lowongan']->tahun_akademik == $year . '-' . ($year + 1) ? 'selected' : '' }}>
                        {{ $year }} - {{ $year + 1 }}
                        </option>
                        @endfor
                </select>
                ...
                <div class="mt-2">
                    <label class="inline-flex items-center">
                        <input type="radio" name="isLogBook" value="1"
                            class="form-radio text-indigo-600 focus:ring-indigo-500 h-4 w-4" required
                            {{$data['lowongan']->isLogBook? 'checked' : ''}} disabled>
                        <span class="ml-2 text-sm text-gray-900">Ya</span>
                    </label>
                    <label class="inline-flex items-center ml-6">
                        <input type="radio" name="isLogBook" value="0"
                            class="form-radio text-red-600 focus:ring-red-500 h-4 w-4" required disabled
                            {{!$data['lowongan']->isLogBook? 'checked' : ''}}>
